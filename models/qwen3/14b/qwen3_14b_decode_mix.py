...
def _validate_decode_config(
    ...
    if total_q_groups % 2 != 0:
        raise ValueError("total_q_groups must be even for gi/gi+1 pairing")
            ...
            for b in pl.parallel(user_batch):
                ...
                for gi in pl.parallel(0, total_q_groups, 2):
                    ...
                    with pl.at(
                        level=pl.Level.CORE_GROUP,
                        optimizations=[pl.auto_chunk, pl.split(pl.SplitMode.UP_DOWN)],
                        name_hint="sv_online_softmax_h0",
                    ):
                        block_table_idx = block_table_base
                        pbid = pl.cast(pl.tensor.read(block_table, [block_table_idx]), pl.INDEX)

                        cache_row0 = (pbid * num_kv_heads + kvh0) * BLOCK_SIZE
                        exp_tile0 = all_exp_padded0[0:Q_HEAD_PAD, :]
                        ...
                        oi0 = pl.mul(pl.matmul(exp_tile0, v_tile0, out_dtype=pl.FP32), 1.0)
                        ...
                        for sb in pl.range(1, ctx_blocks):
                            ...
                            # Keeps the SV matmul result in the layout required by
                            # the following row_expand_mul online update.
                            oi_tmp0 = pl.mul(pl.matmul(exp_tile0, v_tile0, out_dtype=pl.FP32), 1.0)
                            ...
                            oi0 = pl.add(pl.row_expand_mul(oi0, alpha0), pl.row_expand_mul(oi_tmp0, beta0))
                            mi0 = mi_new0
                    ...
                    with pl.at(
                        level=pl.Level.CORE_GROUP,
                        optimizations=[pl.auto_chunk, pl.split(pl.SplitMode.UP_DOWN)],
                        name_hint="sv_online_softmax_h1",
                    ):
                        block_table_idx = block_table_base
                        pbid = pl.cast(pl.tensor.read(block_table, [block_table_idx]), pl.INDEX)

                        cache_row1 = (pbid * num_kv_heads + kvh1) * BLOCK_SIZE
                        exp_tile1 = all_exp_padded1[0:Q_HEAD_PAD, :]
                        v_tile1 = v_cache[cache_row1 : cache_row1 + BLOCK_SIZE, :]
                        # Keeps the SV matmul result in the layout required by
                        # the following row_expand_mul online update.
                        oi1 = pl.mul(pl.matmul(exp_tile1, v_tile1, out_dtype=pl.FP32), 1.0)
                        ...
                        for sb in pl.range(1, ctx_blocks):
                            ...
                            # Keeps the SV matmul result in the layout required by
                            # the following row_expand_mul online update.
                            oi_tmp1 = pl.mul(pl.matmul(exp_tile1, v_tile1, out_dtype=pl.FP32), 1.0)
                            ...
                            oi1 = pl.add(pl.row_expand_mul(oi1, alpha1), pl.row_expand_mul(oi_tmp1, beta1))
                            mi1 = mi_new1
