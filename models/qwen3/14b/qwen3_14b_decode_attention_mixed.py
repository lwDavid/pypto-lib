...
# -----------------------------------------------------------------------------------------------------------
"""Qwen3-14B decode attention mixed — q/k norm + RoPE + paged KV update + decode attention.
...
def build_qwen3_decode_attention_mixed_program(
    batch: int = BATCH,
    max_seq: int = MAX_SEQ,
    ...
    @pl.program
    class Qwen3DecodeAttentionMixed:
        @pl.function(type=pl.FunctionType.Opaque)
        def qwen3_decode_attention_mixed(
            self,
            ...
            for b in pl.range(batch):
                ...
                for gi in pl.range(total_q_groups):
                    ...
                    with pl.at(
                        ...
                        name_hint="qk_store_mix",
                    ):
                        for sb in pl.parallel(ctx_blocks, chunk=MIX_SB_BATCH):
                            ...
                            raw_scores = pl.add(raw_scores, 0.0)
    ...
    return Qwen3DecodeAttentionMixed
...
def golden_qwen3_decode_attention_mixed(tensors):
    import math
    import torch
...
if __name__ == "__main__":
    ...
    result = run(
        program=build_qwen3_decode_attention_mixed_program(),
        specs=build_tensor_specs(use_max_seq=args.max_seq),
        golden_fn=golden_qwen3_decode_attention_mixed,
        config=RunConfig(
            rtol=3e-3,
            atol=3e-3,
